...
function ENT:Draw()
	...
	if LocalPlayer():GetPos():Distance(self:GetPos()) < 500 then
		...
		local pos = self:GetPos() + Vector(0,0,40) - ang:Forward() * (5) - (ang:Right() * (0+ self:GetNWInt("shop_x_offset", -0))) + (ang:Up() * (22 + self:GetNWInt("shop_y_offset", 0)))
		...
		cam.Start3D2D(pos, self.cam2d3dAng, 0.05)
			draw.SimpleText((shopname or "No Name"), "ks5", 22+2, ((60-8-4)/2) - 70+2, Color(0,0,0, 255 * alpha), 1, 1)
